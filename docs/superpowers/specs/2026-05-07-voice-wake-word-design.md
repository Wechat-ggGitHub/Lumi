...
为 Aiva 新增语音唤醒功能：用户说出分身名称（如"钱钱"）即可唤起语音对话，无需按键。唤醒后自动开始录音，VAD 检测静音 3 秒后自动停止，转入豆包 ASR 转写。体验类似 "Hey Siri"。

## Requirements

1. **唤醒词 = 分身名称**：读取 `~/.aiva/persona/profile.json` 的 `name` 字段（默认 "Aiva"），通常是 2-4 个中文字
