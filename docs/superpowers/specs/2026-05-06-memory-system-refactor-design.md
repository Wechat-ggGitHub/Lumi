...
├──────────────────────┬──────────────────────────┤
│   全局核心记忆         │      每日记忆              │
│   SDK Auto-Memory    │   Shrew 自管理             │
├──────────────────────┼──────────────────────────┤
│ 存: ~/.shrew/memories/│ 存: ~/.shrew/daily/       │
...
| `src/lib/shrew-context.ts` | `getActiveMemories()`, `getPinnedMemories()` 函数；`buildShrewContext()` 中移除 `memoryLines` 参数和记忆拼接逻辑 |
| `electron/main.ts` | 6 个 memory IPC handler；`executePrompt()` 中记忆拼接逻辑（`memoryLines` 获取和 `shrewContext` 拼接）；记忆提炼触发逻辑（`extractMemories` 调用） |
...
效果：SDK 的记忆始终读写 `~/.shrew/memories/` 目录，不受工作目录切换影响。SDK 的相关性召回和 /dream 整合继续正常工作。
...
**写入逻辑**:
- 如果 `shouldRecord === true`，将内容追加到 `~/.shrew/daily/YYYY-MM-DD.md`
- 文件格式：
...
**自动注入**（在 `shrew-context.ts` 中）:
- 读取前一天（`YYYY-MM-DD`）的每日记忆文件
- 作为 `## 近期动态` 段落拼接到 shrewContext 中
- 只注入 1 天，保持轻量

**按需读取**:
- 在 shrewContext 的系统提示中追加："每日记忆存储在 `~/.shrew/daily/` 目录，格式为 `YYYY-MM-DD.md`。当用户提及过去发生的事时，用 Read 工具读取对应日期的文件。"
...
**新增**:
- `memory:list-core` — 读取 `~/.shrew/memories/` 目录，返回核心记忆列表
...
- 如果存在，将所有"生效中"的记忆写入 `~/.shrew/memories/` 目录下的独立 .md 文件（使用 type 作为文件名前缀，如 `pref-dark-mode.md`）
- 更新 MEMORY.md 索引
...
| `src/lib/shrew-context.ts` | 修改：移除记忆拼接，改为注入前1天摘要 + 提示 |
