# Shrew - Claude Code 语音壳子 设计文档

## 概述

Shrew 是一个 macOS 桌面应用，通过语音快捷入口驱动 Claude Code 执行任务，并在菜单栏提供状态反馈和执行摘要。
...
- **菜单栏图标**：Shrew logo + 状态小点，常驻
- **语音悬浮窗**：按需弹出，屏幕底部居中（鼠标所在屏幕）
- **摘要弹窗**：点击菜单栏图标后，在图标下方弹出

## 首次启动流程

用户首次打开 Shrew 的完整引导流程：

```
1. 欢迎页
   "Shrew 让你用语音驱动 Claude Code"
   [开始设置]

2. 辅助功能权限引导
   标题："Shrew 需要监听键盘快捷键"
   说明："为了响应右 Command 键唤起语音，Shrew 需要辅助功能权限。
         这与 Raycast、Alfred 等应用所需的权限相同。
         Shrew 只会监听右 Command 键，不会记录任何其他按键。"
   按钮："打开系统设置" → 跳转到 系统设置 → 隐私与安全 → 辅助功能
   用户勾选后自动检测，通过后进入下一步

3. 语音模型下载
   说明："Shrew 使用本地语音识别，需要下载约 230MB 的模型文件"
   进度条 + 下载速度显示
...
- 应用运行时（菜单栏有 Shrew 图标）右 Command 可唤起语音
- 应用退出时快捷键自动注销
...
用户只关心三件事：**Shrew 在干活吗？干完了？出问题了？**

| 应用状态 | SDK 子状态（如有） | 小点 | 用户感知 |
|---|---|---|---|
| idle | - | 灰色静态 | 没事，Shrew 待命中 |
| recording | - | 灰色 | 录音中 |
| transcribing | - | 灰色 | 识别中 |
| editing | - | 灰色 | 编辑中 |
| sending | - | 蓝色脉冲 | 发送中 |
| executing | thinking | 蓝色脉冲 | Shrew 在想 |
| executing | executing_tool | 蓝色脉冲 + tooltip 显示工具名 | Shrew 在干活 |
| executing | compacting | 蓝色脉冲 | Shrew 在整理（用户不用管） |
...
路径：`~/Library/Application Support/Shrew/settings.json`
...
- 模型约 230MB，首次启动时下载，存放在 `~/Library/Application Support/Shrew/models/`
- 中文效果优于 Whisper
