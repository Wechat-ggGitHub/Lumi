...
当前技能管理模块存在根本性问题：`voice-input` 和 `auto-memory` 不是"技能"，而是 Shrew 应用自身功能。真正的技能应该是 Claude Code 风格的 SKILL.md 能力包——模块化的指令集，扩展 Claude 执行任务时的能力。

## 核心决策

| 决策 | 选择 | 原因 |
|------|------|------|
| 数据所有权 | Shrew 独立存储 | 不依赖 Claude Code 的目录结构 |
| 添加方式 | 导入本地 SKILL.md 文件夹 | 复用现有 skill 生态 |
| 执行注入 | 全量注入 systemPrompt.append | 简单可靠，大部分 skill < 2k tokens |
| 旧技能处理 | 移除，始终启用 | 语音输入和自动记忆是应用底层行为 |
| 数据目录 | 统一到 `~/.shrew/` | 与 `config/skills.json` 等分散存储说再见 |
...
│   └── volcengine.json
├── logs/
│   └── shrew-YYYY-MM-DD.log
└── tmp/
    └── recording-*.wav
```

所有数据从 `~/Library/Application Support/Shrew/` 迁移到 `~/.shrew/`。
...
1. **导入**：原生文件选择器选择文件夹 → 复制到 `~/.shrew/skills/`
...
1. 检测 `~/.shrew/` 是否存在
2. 不存在则检查 `~/Library/Application Support/Shrew/`
3. 旧目录存在则按映射关系迁移：
   - `shrew.db` → `~/.shrew/shrew.db`
   - `settings.json` → `~/.shrew/settings.json`（并添加 `disabledSkills: []` 字段）
   - `config/mcp-servers.json` → `~/.shrew/mcp/servers.json`
   - `secure/*` → `~/.shrew/secure/*`
   - `logs/*` → `~/.shrew/logs/*`
   - `config/skills.json` → 丢弃（旧的 voice-input/auto-memory 不再需要）
   - `config/claude.md` → 丢弃（persona+memory 改为通过 SDK systemPrompt 注入）
4. 创建 `~/.shrew/skills/` 空目录
5. 旧目录保留不删除（安全回退）
...
- `electron/main.ts`：`userDataDir` 改为 `~/.shrew/`
- `src/lib/db.ts`：数据库路径改为 `~/.shrew/shrew.db`
- `src/lib/keychain.ts`：加密文件路径改为 `~/.shrew/secure/`
- `src/lib/logger.ts`：日志路径改为 `~/.shrew/logs/`
- `electron/recorder.ts`：临时录音路径改为 `~/.shrew/tmp/`
- `src/lib/shrew-context.ts`：删除 `writeShrewClaudeMd()`（不再写文件），persona+memory 改为通过 SDK `systemPrompt.append` 注入（与 skill catalog 合并）
