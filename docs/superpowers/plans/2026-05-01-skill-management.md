...
**Goal:** 将技能管理从 voice-input/auto-memory 开关重构为 Claude Code SKILL.md 风格的技能系统，并将所有数据统一到 `~/.shrew/` 目录。

**Architecture:** 新增 `skill-manager.ts` 模块处理技能文件系统操作。通过 Claude Agent SDK 的 `systemPrompt.append` 将已启用 skill 注入执行上下文。`userDataDir` 从 Electron `app.getPath('userData')` 改为 `~/.shrew/`。首次启动时自动从旧目录迁移数据。
...
| `src/lib/config-files.ts` | Modify | 删除 skill 函数，MCP 路径改为 `~/.shrew/mcp/` |
| `src/types/index.ts` | Modify | 删除 `SkillConfig`，更新 IPC 类型 |
| `src/app/skills/page.tsx` | Rewrite | 新的技能管理 UI |
| `electron/main.ts` | Modify | `userDataDir` 改为 `~/.shrew/`，替换 skills IPC，添加迁移逻辑 |
| `src/lib/shrew-context.ts` | Modify | 删除 `writeShrewClaudeMd()`，删除文件写入逻辑 |
| `src/lib/keychain.ts` | Modify | `KEYCHAIN_DIR` 改为 `~/.shrew/secure/` |
| `src/lib/logger.ts` | Modify | 无需修改（路径由 main.ts 传入） |
| `electron/recorder.ts` | Modify | `tmpDir` 改为 `~/.shrew/tmp/` |
| `src/lib/memory-extractor.ts` | Modify | 删除 `writeShrewClaudeMd` 调用 |
...
### Task 2: 修改数据目录为 `~/.shrew/`

**Files:**
- Modify: `electron/main.ts:24-26`
- Modify: `src/lib/keychain.ts:9`

- [ ] **Step 1: 修改 `electron/main.ts` — userDataDir 改为 `~/.shrew/`**
...
然后在文件中把所有 `userDataDir` 替换为 `shrewDir`（包括日志初始化、IPC handler 等）。具体需要替换的位置：
- 第 24 行 `const userDataDir = ...` → `const shrewDir = ...`
- 第 608 行 `writeShrewClaudeMd(userDataDir, context)` → 删除这行（Task 4 处理）
...
- 第 627 行 `return loadMcpServers(userDataDir)` → `return loadMcpServers(shrewDir)`
- 第 631-638 行 `addMcpServer(userDataDir, ...)` → `...McpServer(shrewDir, ...)`
- 第 731 行 `initLogger(path.join(userDataDir, 'logs'))` → `initLogger(path.join(shrewDir, 'logs'))`
- 第 735 行 `log.info('userData:', userDataDir)` → `log.info('shrewDir:', shrewDir)`

确保在 `app.whenReady()` 开头创建目录：

```typescript
if (!fs.existsSync(shrewDir)) {
  fs.mkdirSync(shrewDir, { recursive: true });
}
```

- [ ] **Step 2: 修改 `src/lib/keychain.ts` — KEYCHAIN_DIR 改为 `~/.shrew/secure/`**
...
- [ ] **Step 3: 修改 `electron/recorder.ts` — tmpDir 改为 `~/.shrew/tmp/`**
...
- [ ] **Step 4: 修改 `src/lib/config-files.ts` — MCP 路径改为 `~/.shrew/mcp/`**
...
git commit -m "refactor: change data directory from Application Support to ~/.shrew/"
```
...
- [ ] **Step 3: 修改 `src/lib/shrew-context.ts` — 删除 `writeShrewClaudeMd` 和文件写入逻辑**

删除 `writeShrewClaudeMd` 函数（第 67-71 行）及其辅助函数 `getConfigDir`、`ensureConfigDir`、`CONFIG_DIR_NAME`。只保留 `buildShrewContext`、`getActiveMemories`、`getPinnedMemories`。

```typescript
// src/lib/shrew-context.ts — 精简后
...
- [ ] **Step 4: 修改 `src/lib/memory-extractor.ts` — 删除 `writeShrewClaudeMd` 调用**

删除 `import { ..., writeShrewClaudeMd } from './shrew-context'` 中的 `writeShrewClaudeMd`。

删除第 106-112 行的 `writeShrewClaudeMd` 调用：
...
- [ ] **Step 5: 删除 `electron/main.ts` 中 `persona:save` handler 里的 `writeShrewClaudeMd` 调用**
...
- 删除 `writeShrewClaudeMd` import
- 确认 `buildShrewContext`, `getActiveMemories` 仍被 import
...
```bash
git add src/lib/claude-client.ts electron/main.ts src/lib/shrew-context.ts src/lib/memory-extractor.ts
...
app.whenReady().then(async () => {
  ...
  if (!fs.existsSync(shrewDir) && fs.existsSync(oldDir)) {
...
注意：`shrewDir`、`settingsPath`、`dbPath` 在文件顶部已经定义（Task 2 中改过），迁移逻辑在创建目录后执行。需要确保迁移逻辑在 `initLogger` 之前，因为 logger 依赖目录存在。
...
```typescript
  // 确保基础目录存在
  fs.mkdirSync(shrewDir, { recursive: true });
  fs.mkdirSync(path.join(shrewDir, 'logs'), { recursive: true });
  fs.mkdirSync(path.join(shrewDir, 'skills'), { recursive: true });
  fs.mkdirSync(path.join(shrewDir, 'mcp'), { recursive: true });
...
git commit -m "feat: add migration logic from Application Support to ~/.shrew/"
```
