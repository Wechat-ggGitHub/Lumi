...
**两条代码路径**:
- `electron/` — Electron 主进程模块（CJS，由 esbuild 编译到 `dist-electron/`）
- `src/` — Next.js 应用（React 19 页面 + API routes，由 Next.js 构建）
...
| `src/lib/sherpa.ts` | sherpa-onnx SenseVoice 语音识别封装 |
...
```bash
# 开发（Web 模式，无 Electron）
cd Shrew && npm run dev

# 开发（Electron 模式）
cd Shrew && npm run electron:dev

# 构建 Next.js
cd Shrew && npm run build

# 构建 Electron 主进程
cd Shrew && npm run build:electron

# 生产打包（Electron DMG/ZIP）
cd Shrew && npm run electron:build

# 运行测试
cd Shrew && npx jest

# 运行单个测试文件
cd Shrew && npx jest src/__tests__/store.test.ts

# 重新编译 native modules（better-sqlite3, sherpa-onnx 等）
cd Shrew && npm run rebuild
```

## Build Pipeline

1. `next build` → `.next/standalone/` 输出独立服务器
2. `scripts/build-electron.mjs` → esbuild 将 `electron/main.ts` 打包到 `dist-electron/main.js`，native modules 标记为 external
3. `electron-builder` → 打包 DMG/ZIP，将 `.next/standalone` 和 `.next/static` 作为 extraResources 打入 app

## Native Dependencies

项目包含多个需要 `@electron/rebuild` 的 native modules:
- `better-sqlite3` — SQLite 绑定
- `sherpa-onnx-node` — 语音识别推理
...
`@/*` 映射到 `./src/*`（在 tsconfig.json 和 esbuild alias 中均有配置）。
